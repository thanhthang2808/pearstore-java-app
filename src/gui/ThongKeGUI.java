...
public class ThongKeGUI extends JPanel {
    ...
    public ThongKeGUI(TaiKhoan taiKhoanDangNhap) {
        if (taiKhoanDangNhap == null) {
            JOptionPane.showMessageDialog(this, "Bạn cần đăng nhập để truy cập chức năng này!", "Cảnh báo", JOptionPane.WARNING_MESSAGE);
            new LoginGUI();
            return;
        }
        ...
        inputPanel.setBorder(BorderFactory.createTitledBorder(BorderFactory.createLineBorder(primaryColor, 2), "Nhập thông tin báo cáo"));
        ...
        lblNguoiIn = new JLabel(taiKhoanDangNhap.getTenDangNhap());
        ...
        resultPanel.setBorder(BorderFactory.createTitledBorder(BorderFactory.createLineBorder(primaryColor, 2), "Kết quả thống kê"));
    ...
    public void taoBaoCao() {
        try {
            ...
            if (thoiGianBatDau.isEmpty() || thoiGianKetThuc.isEmpty()) {
                JOptionPane.showMessageDialog(this, "Vui lòng chọn đầy đủ thời gian!", "Lỗi", JOptionPane.ERROR_MESSAGE);
                return;
            }
            ...
            Map<Integer, Integer> sanPhamSoLuongBanRa = new HashMap<>();
            Map<Integer, Double> sanPhamDoanhThu = new HashMap<>();
            ...
            List<Map.Entry<Integer, Integer>> sanPhamBanSorted = sanPhamSoLuongBanRa.entrySet()
                    .stream()
                    .sorted(Map.Entry.comparingByValue()) // Sắp xếp tăng dần theo số lượng bán
                    .collect(Collectors.toList());
    
            // Lấy 3 sản phẩm bán chạy nhất
            List<Map.Entry<Integer, Integer>> sanPhamBanChaySorted = sanPhamSoLuongBanRa.entrySet()
                    .stream()
            ...
            List<Integer> maSanPhamBanE = sanPhamBanSorted.stream()
                    .limit(3)
            ...
            ketQua.append("\nSản phẩm bán chạy:\n");
            for (Map.Entry<Integer, Integer> entry : sanPhamBanChaySorted) {
                Integer maSP = entry.getKey();
                SanPham sp = danhSachSanPham.stream().filter(s -> s.getMaSP() == maSP).findFirst().orElse(null);
                if (sp != null) {
            ...
            ketQua.append("\nSản phẩm bán chậm :\n");
            for (Integer maSP : maSanPhamBanE) {
                SanPham sp = danhSachSanPham.stream().filter(s -> s.getMaSP() == maSP).findFirst().orElse(null);
                if (sp != null) {
                    int soLuongBan = sanPhamSoLuongBanRa.get(maSP);
                    double doanhThu = sanPhamDoanhThu.get(maSP);
            ...
            JOptionPane.showMessageDialog(this, "Đã xảy ra lỗi khi tạo báo cáo: " + ex.getMessage(), "Lỗi", JOptionPane.ERROR_MESSAGE);
        }
    }
    ...
    try {
        ...
        baoCao.append("Ngày in: ").append(new SimpleDateFormat("dd/MM/yyyy HH:mm:ss").format(new Date())).append("\n");
        baoCao.append("---------------------------------------------------------------------------\n");
